...
# 模拟各种action下，图片的变化。
# ihone的视网膜屏的分辨率就1920左右。
# 根据hz的感觉，context区域固定256*196 w*h，attention区域可以变化，但是分辨文字只需也就64pix高。
# 底图不放大，否则有内存的问题。
# attention高度24-64. 宽度可以变化，32-196之间. 以8的整倍数
# 考虑 ['up','down','left','right','ahu','ahd','awu','awd','su','sd','rr','rl']
# 这个类按照pytorch的Dataset来写，所以也可以兼容regression的学习。
class text_detection_env(Dataset):
    def __init__(self, image_root_path, label_root_path, if_debug=False):
        ...
        self.sample_num_per_image = 1 # 每个图片可以产生多少个sample
        self.total_len = 20000   # 数据集的总数。由于是online产生的，所以随便设置的。应该为sample_num_per_image*图片数量
        self.counter = 0  # 调用次数的纪录。 
    ...
    become Regression.
    由于每一步都可以由reward，所以PG的learning会退化成Rregression的问题。
    在当前的position下面，获取每个action的reward。
    '''
    def get_all_reward(self):
        rewardlist = []
        for act in self.actlist:
            self.posi = (self.atthei,self.attwid, self.cx, self.cy, self.scaleratio)  # 记录当前位置
            _, reward = self.action(act)
            rewardlist.append(reward)
            self.atthei,self.attwid, self.cx, self.cy, self.scaleratio = self.posi  # 恢复当前的位置
            self.curiou = self.get_max_iou()  # 恢复当前的iou
        return rewardlist    
    ...
    def reset(self):
        ...
        self.atthei = random.choice(self.att_hei_list[0:-2])
        self.attwid = random.choice(self.att_wid_list[0:-2]) #  初始化不要太长
        #self.scaleratio = 1.0
        self.scaleratio = random.uniform(0.5,2.0)  
        #self.rangle = 0.0  
        self.rangle = random.uniform(-5.0,5.0)  
        if self.prompt:
            for i in range(100):
